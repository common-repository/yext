...
function _get_page($host, $path, $internalGetPage) {
  ...
  ob_start();
  $sock = @fsockopen("ssl://".$host, 443, $errno, $errstr, 1);
  if(!$sock) {
    ob_end_clean();
    if ($shouldLog) {
      sendLogMessage('failed to connect, errno: '.$errno.', errstr: '.$errstr);
    }
    return '<!--There was an error connecting to Yext\'s services.-->';
  }
  ...
  if (strpos($response, 'has no associated location id') !== false) {
    return '<!--The widget you are trying to use is not valid. Please update your widget so that it is linked to a specific location.-->';
  } elseif ($statusCode == 400) {
    return '<!--The widget you are trying to use is not valid. Please make sure you have the correct id and type in your shortcode.-->';
  } elseif ($statusCode == 404) {
    return '<!--The widget you are trying to use cannot be found.-->';
  }

  return '<!--There was an error loading your widget. If this issue persists, please contact Yext support.-->';
}
...
?>
  ...
  <script type="text/javascript">
    ...
    window.receiveMessage = function(event) {
      // Validate origin of Post Message. First checks if the origin is from yext, in which case it
      // is checked against the iframes to validate that it came from within the page.
      // Then checks whether the origin is the same as the wordpress origin, to handle cases like
      // "localhost:portNumber".
      let isFromSamePage = false;
      if(event.origin === 'https://www.yext.com') {
      ...
      if (!isFromSamePage
          ...
          && window.location.protocol+"//"+window.location.hostname !== event.origin)
      {
          return
      }
